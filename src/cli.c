...
static const char *esq_seq_insert_char = "\x1B[@";

// Escape sequence - Cursor delete charcater (DCH)
...
const struct error_code_lut_row error_code_lut[] = {
        ...
        CLI_ERR_DESC(CLI_DBG_LVL_WARN, UNKNOWN_ESC_CHAR),
...
CLI_ERR _insert_char_under_cursor(Cli* state, char c){
        ...
        if(state->head != state->hcursor){
                state->write_data(esq_seq_insert_char);
        }
        if (state->write_data(buff)!=0){
                        return CLI_ERR_WRITE;
        }
...
CLI_ERR _execute_command_buff(Cli* state){
        ...
        if (state->head > 0){
                strncpy(state->history[state->history_head], state->linebuff, BUFF_MAX_CHARS);
                state->history_head = (state-> history_head + 1) % MAX_HISTORY;

                if (state->history_count < MAX_HISTORY){
                        state->history_count++;
                }
                state->history_index = state->history_head;
        }
...
CLI_ERR _navigate_history(Cli* state, int dir){
        ...
        if (state->history_count == 0){
                state->write_data("No history to recall\n\r");
                return CLI_ERR_OK;
        }
        int new_index = state->history_index + dir;
        if (new_index < 0 || new_index >= state->history_count){
                return CLI_ERR_OK;
        }
        state->history_index = new_index;
        int real_index = (state->history_head - state->history_count + new_index + MAX_HISTORY) % MAX_HISTORY;
        state->write_data(esc_seq_delete_line);
        state->write_data("\r");

        strncpy(state->linebuff, state->history[real_index], BUFF_MAX_CHARS);
        state->head = strlen(state->linebuff);
        state->hcursor = state->head;
        state->write_data(state->linebuff);
};
